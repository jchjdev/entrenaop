-- Circuitos con transición por estación y Tabata con ocho posiciones libres.
...
begin
  ...
  loop
    ...
    if block_format = 'intervals' and block_exercise_count <> 1 then
      raise exception 'Intervals require exactly one exercise';
    end if;
    if block_format = 'tabata' and block_exercise_count <> 8 then
      raise exception 'Tabata requires exactly eight work positions';
    end if;
    ...
    loop
      ...
      if block_format in ('superset', 'circuit', 'intervals', 'emom')
      ...
      if block_format in ('tabata', 'amrap')
        and jsonb_array_length(exercise_json -> 'sets') <> 1 then
        raise exception 'This format requires one target per position';
      end if;
...
begin
  ...
  select
    ...
    case
      ...
      when block.format = 'tabata' and item.order_index < (
        select max(last_item.order_index)
        from public.workout_items as last_item
        where last_item.block_id = block.id
      ) then 10
      when block.format = 'tabata' then 0
      when block.format = 'circuit' and item.order_index = (
        select max(last_item.order_index)
        from public.workout_items as last_item
        where last_item.block_id = block.id
      ) then block.rest_after_seconds
      when block.format = 'circuit' then prescribed_set.rest_after_seconds
      when block.format in ('superset', 'intervals')
        and item.order_index = (
          select max(last_item.order_index)
          from public.workout_items as last_item
          where last_item.block_id = block.id
        ) then block.rest_after_seconds
      when block.format in ('superset', 'intervals') then 0
