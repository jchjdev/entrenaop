-- Intervalos configurables y Tabata canónico. Las mismas reglas que aplica la
-- app se validan aquí para que un cliente modificado no pueda crear formatos
-- incoherentes.
...
begin
  ...
  loop
    ...
    if block_format not in (
      'straight_sets', 'superset', 'circuit', 'intervals', 'tabata'
    ) then
    ...
    loop
      ...
      if block_format in ('superset', 'circuit', 'intervals', 'tabata')
